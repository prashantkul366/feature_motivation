...
VENDORED from https://github.com/google/svcca (commit 1f3fbf1), file cca_core.py.
Reference: Raghu et al., "SVCCA: Singular Vector Canonical Correlation Analysis
for Deep Learning Dynamics and Interpretability", NeurIPS 2017.
https://arxiv.org/abs/1706.05806

--------------------------------------------------------------------------
LOCAL MODIFICATIONS (2 upstream bugs in `robust_cca_similarity` only;
`get_cca_similarity` is byte-identical to upstream):

  [PATCH 1] `np.LinAlgError` -> `np.linalg.LinAlgError`.
            `np.LinAlgError` has never existed in any numpy version. The
            original raises AttributeError instead of retrying.

  [PATCH 2] `get_cca_similarity(acts1, acts2, threshold, compute_dirns)` was
            called with POSITIONAL args against the signature
            `(acts1, acts2, epsilon, threshold, compute_coefs, compute_dirns)`,
            so `threshold` silently landed in the `epsilon` slot and
            `compute_dirns` landed in the `threshold` slot. Now keyword args.

Both patches are marked inline with `# [PATCH n]`.

NOTE ON USAGE: `get_cca_similarity` computes plain CCA. Full SVCCA is
SVD/PCA truncation FOLLOWED BY CCA. In this repo the truncation step is the
explicit PCA in analysis/similarity.py, so PCA(k) -> get_cca_similarity == SVCCA.
--------------------------------------------------------------------------
"""
...
def robust_cca_similarity(acts1, acts2, threshold=0.98, epsilon=1e-6,
  ...
  for trial in range(num_cca_trials):
    try:
      # [PATCH 2] upstream passed these POSITIONALLY, which put `threshold`
      # into the `epsilon` slot and `compute_dirns` into the `threshold` slot.
      return_dict = get_cca_similarity(
          ...
          verbose=verbose)
      break
    except np.linalg.LinAlgError:  # [PATCH 1] upstream: np.LinAlgError
